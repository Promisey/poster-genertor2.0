...
function App() {
  ...
  const styleTemplates = {
    professional: {
      ...
      textColor: '#FFFFFF',
      productAreaBackgroundColor: '#F8F8F8' // 浅灰色
    },
    fresh: {
      ...
      textColor: '#FFFFFF',
      productAreaBackgroundColor: '#F0FFF0' // 浅绿色
    },
    warm: {
      ...
      textColor: '#FFFFFF',
      productAreaBackgroundColor: '#FFF5EE' // 浅橙色
    },
    elegant: {
      ...
      textColor: '#FFFFFF',
      productAreaBackgroundColor: '#F5F3FF' // 浅紫色
    },
    vibrant: {
      ...
      textColor: '#FFFFFF',
      productAreaBackgroundColor: '#FFF7ED' // 浅橙色
    }
  }
  ...
  const layoutParams = {
    ...
    baseHeaderHeight: 80, // 基础头部高度（店铺名称和标语区域）
    ...
    imageToInfoAreaGap: 10, // 图片与信息区域的间距 (调整)
    textLineSpacing: 4, // 文字行间距
    priceLineHeight: 18, // 价格行高 (调整)
  ...
  const calculateProductLayout = (templateKey) => {
    ...
    // 计算商品图片尺寸（正方形）
    const availableWidthForImage = cardWidth - 2 * cardInnerPadding
    
    // 图片尺寸：确保为正方形，并充分利用可用宽度
    const imageSize = Math.max(60, Math.min(availableWidthForImage * 0.9, availableWidthForImage))
    ...
    if (posterConfig.headerImage) {
      headerHeight += 100 // 宣传图片高度，这里只是预留空间，实际绘制时会自适应
    }
  ...
  const drawHeader = (ctx, style, headerHeight) => {
    ...
    if (posterConfig.headerImage) {
      ...
      img.onload = () => {
        ...
        if (targetHeight < baseHeaderHeight) { // 如果图片高度小于店铺信息区域高度，则拉伸或裁剪
          drawHeight = baseHeaderHeight
          drawWidth = baseHeaderHeight * imgAspectRatio
          drawX = (posterWidth - drawWidth) / 2
        } else if (targetHeight > baseHeaderHeight) { // 如果图片高度大于店铺信息区域高度，则裁剪
          drawY = (targetHeight - baseHeaderHeight) / -2
        }

        ctx.drawImage(img, drawX, drawY, drawWidth, drawHeight)

        // 绘制店铺名称和标语
        drawStoreInfo(ctx, style, currentY, posterWidth)
      ...
      // 如果没有宣传图片，绘制渐变背景
      const gradient = ctx.createLinearGradient(0, currentY, posterWidth, currentY + baseHeaderHeight)
      gradient.addColorStop(0, style.primaryColor)
      gradient.addColorStop(1, style.secondaryColor)
      
      ctx.fillStyle = gradient
      ctx.fillRect(0, currentY, posterWidth, baseHeaderHeight)
  ...
  // 绘制店铺名称和标语的辅助函数
  const drawStoreInfo = (ctx, style, startY, posterWidth) => {
    ctx.fillStyle = style.textColor
    ctx.font = 'bold 24px Arial, sans-serif'
    ctx.textAlign = 'center'
    ctx.fillText(posterConfig.storeName, posterWidth / 2, startY + 35)

    ctx.font = '14px Arial, sans-serif'
    ctx.fillText(posterConfig.storeSlogan, posterWidth / 2, startY + 60)
  }
      ...
      if (product.image) {
        if (product.image.startsWith('http')) {
          ...
          img.onload = () => {
            ...
            ctx.restore() // 恢复画布状态
            // 绘制商品信息
            drawProductInfo(ctx, product, cardX, cardY, cardWidth, cardHeight, imageY + imageSize, fontSize, calculateInfoHeight)
          }
          img.onerror = () => {
  ...
  // 绘制商品信息的辅助函数 (调整)
  const drawProductInfo = (ctx, product, cardX, cardY, cardWidth, cardHeight, imageBottomY, fontSize, calculateInfoHeight) => {
    ...
    const totalPricesWidth = originalPriceWidth + priceWidth + 5 // 5px为价格间距
    let currentPriceX = contentX + (contentWidth - totalPricesWidth) / 2

    // 划线价
    if (product.originalPrice && parseFloat(product.originalPrice) > 0) {
      ctx.fillStyle = '#9CA3AF'
      ctx.font = `${fontSize.originalPrice}px Arial, sans-serif`
      ctx.fillText(originalPriceText, currentPriceX + originalPriceWidth / 2, textStartY + priceLineHeight / 2)
      
      // 绘制删除线
      ctx.strokeStyle = '#9CA3AF'
      ctx.lineWidth = 1
      ctx.beginPath()
      ctx.moveTo(currentPriceX, textStartY + priceLineHeight / 2 - fontSize.originalPrice / 3)
      ctx.lineTo(currentPriceX + originalPriceWidth, textStartY + priceLineHeight / 2 - fontSize.originalPrice / 3)
      ctx.stroke()

      currentPriceX += originalPriceWidth + 5
    }

    // 促销价
    ctx.fillStyle = '#EF4444'
    ctx.font = `bold ${fontSize.price}px Arial, sans-serif`
    ctx.fillText(priceText, currentPriceX + priceWidth / 2, textStartY + priceLineHeight / 2)
  }
