...
function App() {
  ...
  const layoutParams = {
    ...
    baseFooterHeight: 120, // 增大基础底部高度（从80增加到120）
  ...
  const calculateProductLayout = (templateKey) => {
    ...
    const imageSize = Math.max(60, Math.min(availableWidthForImage * 0.85, availableWidthForImage))
  ...
  const drawHeader = async (ctx, style, headerHeight) => {
    ...
    if (posterConfig.headerImage) {
      await new Promise((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error('宣传图片加载失败'))
  ...
  const drawProductInfo = (ctx, product, cardX, cardY, cardWidth, cardHeight, imageBottomY, fontSize, calculateInfoHeight) => {
    ...
    const totalPricesWidth = originalPriceWidth + priceWidth + 8 // 8px为价格间距
    ...
    if (product.originalPrice && parseFloat(product.originalPrice) > 0) {
      ...
      ctx.font = `${fontSize.originalPrice}px Arial, sans-serif`
      ctx.textAlign = 'left'
      ctx.fillText(originalPriceText, currentPriceX, textStartY + priceLineHeight / 2)
      ...
      currentPriceX += originalPriceWidth + 8
    }
    ...
    ctx.font = `bold ${fontSize.price}px Arial, sans-serif`
    ctx.textAlign = 'left'
    ctx.fillText(priceText, currentPriceX, textStartY + priceLineHeight / 2)
  }
  ...
  const drawFooter = async (ctx, posterHeight) => {
    ...
    if (posterConfig.qrCode) {
      await new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          try {
            const qrSize = 60 // 增大二维码尺寸
            const qrX = posterWidth / 2 - qrSize / 2
            const qrY = footerY + 15 // 调整位置
        ...
        img.onerror = () => {
          drawQRPlaceholder(ctx, footerY, posterWidth)
          resolve()
        }
    ...
    // 绘制底部文字
    ctx.fillStyle = '#6B7280'
    ctx.font = '14px Arial, sans-serif' // 增大字体
    ctx.textAlign = 'center'
    ctx.fillText(posterConfig.bottomText, posterWidth / 2, footerY + baseFooterHeight - 20)
  }

  // 绘制二维码占位符
  const drawQRPlaceholder = (ctx, footerY, posterWidth) => {
    const qrSize = 60
    const qrX = posterWidth / 2 - qrSize / 2
